...
(function () {
    ...
    function instrumentAssignmentExprStatement(node, idx, scope) {
		...
		if(node.right.type === Syntax.UpdateExpression) {
			var updateExprVarName = node.right.argument.name,updateExprVar;
			if (scope.length == 1)
				updateExprVar = b.memberExpression(cs, b.literal(updateExprVarName), true);
			else
				updateExprVar = common.getFindVarInScopeChainExpr(cs, updateExprVarName, null, b.literal(true));
			var condExpr = common.getLubFuncCallExprWithArgsArray(
				[common.secLvlHelper(b.identifier(updateExprVarName), false, cs),pcDotL]);
