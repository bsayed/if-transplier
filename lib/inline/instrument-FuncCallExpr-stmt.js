...
(function () {
    ...
    function instrumentFuncCallExprStatement(node, idx, scope) {
        ...
        if (!isMethodCall && node.type !== Syntax.NewExpression)
            ...
            common.Map['' + scope.concat(lhsName)] = '' + scope.concat(rhsName+'.prototype');
        ...
        if(params === undefined) {
            ...
            if(rhsName[0] === 'this') {
                if(scopeCopy.length == 1) {
                    ...
                    scopeCopy.pop();
                    proto = common.Map[''+scopeCopy[scopeCopy.length-1]];
                    if(proto)
                        scopeCopy[scopeCopy.length-1] = proto;
                    params = common.Map[''+scopeCopy.concat(rhsName[1])]
                }
            } else {
                proto = common.Map['' + scope.concat(rhsName[0])];
                if (proto) {
                    params = common.Map[proto + ',' + rhsName[1]];
                }
            }
        }
